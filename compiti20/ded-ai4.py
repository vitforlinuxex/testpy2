...
# Aggiungiamo a Mago e Guerriero il metodo subisci_danno (già implementato)

# Bacchetta dei fulmini derivata dal codice dato, adattata
class Bacchetta_dei_fulmini:
...
def main():
    ...
    bacchetta = Bacchetta_dei_fulmini()
    spada = None  # Non necessario come classe separata qui
    ...
    while True:
        ...
        if troll.vitalita > 0:
            ...
            if not bersagli:
                ...
                bersaglio = seleziona_bersaglio(bersagli)
                danno = dado
                # Troll subisce più danni se maledetto, subisce meno forza ordinarie non trattate qui
                troll.subisci_danno = troll.subisci_danno.__get__(troll)
                bersaglio.subisci_danno(danno)
